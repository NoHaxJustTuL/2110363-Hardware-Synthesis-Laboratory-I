...
async def tb_1(dut):
    ...
    await Timer(10, units="ns")
    assert dut.lfsr_out == 0xffff, "Reset Fail: LFSR should be 0xFFFF after reset"
...
async def tb_2(dut):
    ...
    for i in range(16):
        dut.enable.value = 1
        await Timer(10, units="ns")
        assert dut.lfsr_out == expected_output[i], f"LFSR Test Fail: Expected {expected_output[i]:#06x}, got {dut.lfsr_out:#06x}"
        dut.enable.value = 0
        await Timer(10, units="ns")
